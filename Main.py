...
async def start(_, message: Message):
    await message.reply_text("👋 Send /login <your_phone_number> to login to your Telegram account.")
...
async def handle_login(_, message: Message):
    ...
    if len(args) != 2:
        await message.reply_text("📱 Use `/login <your_phone_number>`\nExample: `/login +919876543210`", quote=True)
        return
...
async def login_flow(_, message: Message):
    ...
    if step == "otp":
        try:
            ...
            session["step"] = "2fa"
            await message.reply_text("🔐 2FA is enabled. Please enter your password:")
        except Exception as e:
